...
import { useProgress, useUI } from '@/lib/state';
...
function ControlTray({ children }: ControlTrayProps) {
  ...
  const { advanceStage, isLastStage } = useProgress();
  const lastStage = isLastStage();
  ...
  return (
    <section className="control-tray">
      ...
      <button
        className="button next-stage-button"
        onClick={advanceStage}
        disabled={lastStage || !connected}
      >
        Next Stage <span className="icon">arrow_forward</span>
      </button>
    </section>
  );
}
