...
export class AudioStreamer {
  ...
  private initialBufferTime: number = 0.2; // 200ms initial buffer
  ...
  /**
   * Calculates the total duration of audio currently in the queue.
   * @returns The buffered duration in seconds.
   */
  private getBufferedDuration(): number {
    return this.audioQueue.reduce(
      (duration, buffer) => duration + buffer.length / this.sampleRate,
      0
    );
  }
  ...
  addPCM16(chunk: Uint8Array) {
    ...
    // Start playing if not already playing and we have buffered enough audio.
    if (
      !this.isPlaying &&
      this.getBufferedDuration() >= this.initialBufferTime
    ) {
      this.isPlaying = true;
      // Initialize scheduledTime to the current time, as we've already buffered.
      this.scheduledTime = this.context.currentTime;
      this.scheduleNextBuffer();
    } else if (this.isPlaying && this.checkInterval) {
      // If we are playing, but the scheduler was idle (polling for data),
      // we just received data, so we should kick off scheduling immediately
  ...
  private scheduleNextBuffer() {
    // If this function is running, it means we're actively scheduling.
    // We can clear the polling interval if it exists, since we're no longer polling.
